...
class SrtClient(private val connectChecker: ConnectChecker) {
  ...
  /**
   * The TSBPD latency budget this end asks for, in MILLISECONDS. See the note at the handshake
   * below for what it buys and how it is negotiated. Set it before [connect].
   *
   * ⚠ **MILLISECONDS, AND THE WIRE FIELD IS 16 BITS.** SRT latency is quoted in microseconds in
   * some places — the `latency=` query parameter of an SRT url is microseconds, so `500000`
   * there is half a second. THIS IS NOT THAT. This value goes into the handshake extension as a
   * 16-bit millisecond field, and `writeUInt16` keeps the low 16 bits of whatever it is given
   * WITHOUT complaining. Half a second written here as `500000` would go out as 41248 ms: a
   * stream forty seconds behind reality, from a number that looks correct.
   *
   * `VideoTransport.clampLatencyMs` clamps before it reaches this, and every application call
   * site goes through it, so the corruption cannot arrive from a setting. This note is for
   * anyone setting the field from new code.
   *
   * The default below is only what an unconfigured client would use — the application always
   * sets this. The evidence for the number is at `VideoTransport.SRT_LATENCY_DEFAULT_MS`, and
   * it is repeated there rather than shared with it so this vendored module keeps depending on
   * nothing outside itself.
   */
  var latencyMs: Int = 500
  ...
  /**
   * ⚠ NOT IMPLEMENTED BY THE LIBRARY, in any published version. The body was `TODO()`, thus a
   * call THREW at run time with no message.
   *
   * SRT has no user/password field. The credentials go in the stream id, which this client
   * takes from the path part of the url:
   *
   *     srt://host:8890/publish:<path>:<user>:<password>
   *
   * [AutelVideoStreamer.VideoConfig.pushUrl] builds that. Do not call this function.
   */
  fun setAuthorization(user: String?, password: String?): Nothing =
    throw UnsupportedOperationException(
      "SRT has no user/password field — put the credentials in the stream id. See pushUrl().")
        ...
        val error = runCatching {
          ...
          commandsManager.writeHandshake(socket, response.copy(
            ...
            handshakeExtension = HandshakeExtension(
              ...
              // TAKPILOT2 CHANGE: the library sends a fixed 120 ms. SRT recovers a lost packet
              // by asking for it again, thus this buffer is the time it has to do that before
              // the receiver must show the frame. A repair costs about ONE MORE ROUND TRIP, so
              // the buffer has to hold several times the RTT or the repaired packet arrives
              // after its own deadline and is thrown away.
              //
              // The two ends negotiate this: the effective value is the LARGER of the two
              // sides, so a server that asks for more still gets what it asked for.
              //
              // ⚠ BOTH FIELDS, AND THE SENDER ONE IS THE LOAD-BEARING ONE. The handshake
              // carries a Receiver TSBPD delay and a Sender TSBPD delay. This client is a
              // PUBLISHER: it receives no media, so its receiver delay governs nothing, and
              // the field that tells the server how long to hold OUR video is the sender one.
              //
              // The library sent `senderDelay = 0`. The server then used its own default,
              // and every latency value set here was inert. Measured in flight against
              // MediaMTX v1.20.0 on 2026-08-29: msSendTsbPdDelay 500 (our number, echoed on
              // a direction that carries nothing) against msReceiveTsbPdDelay 120 (the
              // server's default, holding the stream that matters).
              //
              // Setting both is what libsrt's own SRTO_LATENCY does.
              //
              // Milliseconds — see [latencyMs].
              receiverDelay = latencyMs,
              senderDelay = latencyMs,
              path = path,
